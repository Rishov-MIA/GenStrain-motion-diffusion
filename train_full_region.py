"""Full-region-noise training, conditioned on the contour mask only.

All hyperparameters live in a JSON config (default: configs/train_full_region.json).
Run a different experiment by pointing at another config:

    python train_full_region.py --config configs/my_experiment.json
...
DEFAULT_CONFIG = Path(__file__).resolve().parent / "configs" / "train_full_region.json"
