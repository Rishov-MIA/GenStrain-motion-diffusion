"""Contour-region-noise training, conditioned on both the contour mask and motion.

All hyperparameters live in a JSON config (default: configs/train_both_contour_motion.json).
Run a different experiment by pointing at another config:

    python train_both_contour_motion.py --config configs/my_experiment.json
...
DEFAULT_CONFIG = Path(__file__).resolve().parent / "configs" / "train_both_contour_motion.json"
