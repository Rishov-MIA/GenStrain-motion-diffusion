...
class Unet3D(nn.Module):
    ...
    def forward(
        ...
        x = self.init_temporal_attn([x, x], x, pos_bias=time_rel_pos_bias)
        ...
        h = []
        # count = 0
        control_residuals = default(control_residuals, [])
        for layer_idx, (block1, block2, spatial_attn, temporal_attn, downsample) in enumerate(self.downs):
            x = block1(x, t)
            x = block2(x, t)
            ...
            if layer_idx < len(control_residuals) and exists(control_residuals[layer_idx]):
                x = x + control_residuals[layer_idx] * control_scale
        ...
        x = self.mid_temporal_attn(
            [x, x], x, pos_bias=time_rel_pos_bias, focus_present_mask=focus_present_mask
        )

        x = self.mid_block2(x, t)

        if exists(mid_control_residual):
            x = x + mid_control_residual * control_scale
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, cond=None, cond_scale=1.0, batch_size=16):
        ...
        elif exists(cond) and len(cond) == 1 and isinstance(cond[0], torch.Tensor):  # video
            cond = [cond[0].to(device)]
    ...
    def forward(self, x, cond=None, *args, **kwargs):
        ...
        if exists(cond) and len(cond) == 1 and isinstance(cond[0], torch.Tensor):
            cond = [normalize_divide_by_5(cond[0])]
        return self.p_losses(x, t, cond=cond, **kwargs)
...
class Dataset(data.Dataset):
    def __init__(
        ...
        self.video_paths = sorted(input_video_paths)
    ...
    def __getitem__(self, index):
        input_video_path = self.video_paths[index]
        ...
        if index not in self.cache:
            ...
            # Cache the data
            self.cache[index] = input_video_tensor
        else:
            # Retrieve from cache
            input_video_tensor = self.cache[index]

        # Optionally, you can apply any transformations here (like resizing, normalization, etc.)
        # For now, return the tensors directly
        return input_video_tensor.squeeze(0)  # Remove extra dimension
...
class Trainer(object):
    ...
    def sample_and_save(
        ...
        sampled_videos = self.ema_model.sample(cond=None, cond_scale=cond_scale, batch_size=num_samples)
