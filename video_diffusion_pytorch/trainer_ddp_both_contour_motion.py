...
class BothCondDDPTrainer(Trainer):
    ...
    def _disp_metric_names(self):
        ...
        present; the ratio depends on the model's disp_rel_metric form. Empty when
        disp_metrics is off, so a reproduction run declares no panel it will never
        send a value to."""
        if not getattr(self.raw_model, "disp_metrics", True):
            return []
        names = ["gt_disp_msq"]
